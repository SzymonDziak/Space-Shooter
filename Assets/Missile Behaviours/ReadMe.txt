...
The scripts:
	...
	Control scripts:
		...
		- TailChaseGuidance.cs:
		  ...
		  at first, swing back and then tail it. Sometimes it even misses the second approach. This way is far less effective than proportional navigation because it has a lower chance to hit and
		  will usually take much longer to even reach the target.

		- BallisticGuidance.cs:
		  This script alligns the missiles rotation with its travel vector under certain circumstances.
	...
	Trigger scripts:
		...
		- OnLevelFlight.cs
		  This script triggers when the missiles rotation get close to 0/360 degrees. Therefor it will trigger if the missile is in level flight.
	...
	Action scripts:
		...
		- AltitudeControl.cs
		  This script is a sort of waypoint system, which uses an imaginary target for the missile to reach. Once reached it will either change to another imaginary target, or waypoints, or switch to the actual target.
		  This way a missile can, for example, fly straight up, approach the target on height and then drop straight on its head in order to avoid obstacles or make it harder to be detected or shot down.
...
Layer 12: GroundTargets

Additionally, Unity does not save the physics settings within the contents of a package. That means things that shouldn't be able to collide, now might. I suggest disableing Missile-Missile collision and (for the Dreadnought demo)
