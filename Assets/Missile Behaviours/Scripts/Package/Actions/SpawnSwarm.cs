...
namespace MissileBehaviours.Controller
{
    ...
    public class SpawnSwarm : ActionBase
    {
        ...
        int spawnCount;
        [SerializeField, Tooltip("The axis to use.")]
        Axis axis;
            ...
            if (!wasTriggered && this.enabled)
            {
                ...
                for (int i = 0; i < spawnCount; i++)
                {
                    ...
                    if (axis == Axis.Forward)
                        rotation = Quaternion.AngleAxis(i * step, transform.forward);
                    if (axis == Axis.Right)
                        rotation = Quaternion.AngleAxis(i * step, transform.right);
                    if (axis == Axis.Up)
                        rotation = Quaternion.AngleAxis(i * step, transform.up);

                    rotation = rotation * transform.rotation * Quaternion.LookRotation(Vector3.right) * Quaternion.AngleAxis(-angle, Vector3.up);
